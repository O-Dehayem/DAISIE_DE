...
Likelihood_EC_lineage <- function(datalist, i, pars1) {
  ...
  interval2 <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
      ...
      dD0 <- -pars1[4] * D0 + pars1[4] * Dm
      dDm <- -(pars1[5] + pars1[1] + pars1[2]) * Dm + (pars1[5] * E1 + pars1[1] * E1^2 + pars1[2]) * D0 
      dDM <- -(pars1[5] + pars1[1] + pars1[2] + pars1[4]) * DM + (pars1[5] * D1 + 2 * pars1[1] * D1 * E1) * D0
      dE1 <- pars1[2] - (pars1[1] + pars1[2]) * E1 + pars1[1] * E1^2
  ...
  # Initial conditions
  initial_conditions1 <- c(D1 = 1, D0 = 1, Dm = 0, E1 = 0)
